...
public class FrontendMain {
    ...
    public static void main(String[] args) {
        ...
        Spark.get("/search/:topic", (req, res) -> {
            ...
            String replace = "http://172.17.0.2:" + CATALOG_PORT + "/search/" + topic.replace(" ", "%20");
            return forwardRequest( replace,"GET");
        });

        Spark.get("/searchItem/:itemName", (req, res) -> {
            String itemName = req.params(":itemName");
            return forwardRequest("http://172.17.0.2:" + CATALOG_PORT + "/searchItem/" + itemName.replace(" ", "%20"), "GET");
        });

        Spark.get("/info/:itemNumber", (req, res) -> {
            String itemNumber = req.params(":itemNumber");
            return forwardRequest("http://172.17.0.2:" + CATALOG_PORT + "/info/" + itemNumber, "GET");
        });

        Spark.post("/purchase/:itemNumber", (req, res) -> {
            String itemNumber = req.params(":itemNumber");
            return forwardRequest("http://172.17.0.3:" + ORDER_PORT + "/purchase/" + itemNumber, "POST");
        });
        ...
        Spark.post("/updateCost/:itemNumber/:newCost", (req, res) -> {
            ...
            return forwardRequest("http://172.17.0.2:" + CATALOG_PORT + "/update/cost/" + itemNumber + "/" + newCost, "POST");
        });

        Spark.post("/increaseStock/:itemNumber/:amount", (req, res) -> {
            String itemNumber = req.params(":itemNumber");
            String amount = req.params(":amount");
            return forwardRequest("http://172.17.0.2:" + CATALOG_PORT + "/update/increaseStock/" + itemNumber + "/" + amount, "POST");
        });

        Spark.post("/decreaseStock/:itemNumber/:amount", (req, res) -> {
            String itemNumber = req.params(":itemNumber");
            String amount = req.params(":amount");
            return forwardRequest("http://172.17.0.2:" + CATALOG_PORT + "/update/decreaseStock/" + itemNumber + "/" + amount, "POST");
        });
    }
